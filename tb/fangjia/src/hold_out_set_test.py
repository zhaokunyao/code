...
class HoldOutSetTest(object): 
    ...
    def trainAndValidate(self, X_train, X_test, y_train, y_test): 
        ...
        for (classifier_name, clsf) in self.arrClassifiers.items(): 
            ...
            self.arrModels[classifier_name] = model
    ...
    def pickle(self): 
        """pickle
        """
        pickle.dump(self.arrModels, open("class.dat", "wb"), True)

    def go(self): 
        """go
        """
        X_train, y_train = load_svmlight_file('./data/train.svm')
        X_test, y_test = load_svmlight_file('./data/test.svm')
